					...
					<tbody>
						<c:forEach items="${article}" var="item">
							<tr>
								...
								<td><a href="../article/detail?articleNo=${item.articleNo}">${item.articleSubject}</a></td>
								<td>${item.articleWriter}</td>
								<td>${item.articleHit}</td>
								<td>${item.articleUpdateDay}</td>
							</tr>
						</c:forEach>
					</tbody>
					<tfoot>
						<tr>
							<td colspan="4"></td>
							<td><a href="../article/insertform" class="button" id="write">글쓰기</a></td>
						</tr>
					</tfoot>
				</table>
			</div>
			<div class="align-center">
				<ul class="pagination">
					<c:if test="${paging.curGroup >= 1}">
						<li><a href="#" onclick="page('1')">Start</a></li>
					</c:if>
					<c:if test="${paging.curGroup >= 1}">
						<li><a href="#" onclick="page(${paging.prevPage})">Prev</a></li>
					</c:if>
					...
					<c:if test="${paging.curGroup <= paging.totGroup}">
						<li><a href="#" onclick="page(${paging.nextPage})">Next</a></li>
					</c:if>
					<c:if test="${paging.curPage <= paging.totPage}">
						<li><a href="#" onclick="page(${paging.totPage})">End</a></li>
					</c:if>
				</ul>
			</div>
		</div>
